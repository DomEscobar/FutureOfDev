...
class TacticalExecutor {
    ...
    decideForLoginCheck(elements, state) {
        // Check if we're already logged in (on dashboard or have logout button)
        const urlPath = state?.urlPath?.toLowerCase() || '';
        const hasLogout = elements.some(e => 
            e.raw.toLowerCase().includes('logout') || 
            e.raw.toLowerCase().includes('sign out')
        );
        
        if (urlPath.includes('dashboard') || urlPath.includes('roster') || hasLogout) {
    ...
    async execute(subtask, maxSteps = 10) {
        ...
        for (let step = 0; step < maxSteps; step++) {
            ...
            if (subtask.type === 'ensure_logged_in' && result.navigated) {
                completed = true;
                break;
            }
