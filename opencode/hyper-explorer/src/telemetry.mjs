...
// Load config.json from opencode folder
let config = {};
try {
  const configPath = '/root/FutureOfDev/opencode/config.json';
  config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
} catch (e) {
  // Config not found
}

// Telegram config - config.json takes precedence over ENV
const TELEGRAM_TOKEN = config.TELEGRAM_BOT_TOKEN || process.env.TELEGRAM_BOT_TOKEN || '';
const TELEGRAM_CHAT_ID = config.TELEGRAM_CHAT_ID || process.env.TELEGRAM_CHAT_ID || '';
